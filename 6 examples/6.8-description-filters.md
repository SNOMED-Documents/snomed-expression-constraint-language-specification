...
Dialect filters enable an expression constraint to match on only those concepts with a matching description in a specified language reference set. Dialect filters may either use the keyword "dialect" with a value that represents a valid alias for a specific language reference set, or may use the keyword "dialectId" with a concept value that is < [900000000000506000 |Language type reference set|](http://snomed.info/id/900000000000506000) . Please refer to [appendix-c-dialect-aliases.md](../appendix-c-dialect-aliases.md "mention") for a selection of valid dialect aliases for known language reference sets.
