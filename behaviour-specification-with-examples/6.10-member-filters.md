...
Active filters enable an expression constraint to match on only those members of a reference set with a matching active status. Reference set rows are either active (i.e. active = 1 or active = "true"), inactive (i.e. active = 0 or active = "false"), or either (i.e. active = "\*"). By default, only active members of a reference set are included in the substrate.&#x20;

For example, the following expression constraint returns the inactive members of the  `816080008 | International Patient Summary|` .

```
^ 816080008 |International Patient Summary| {{ M active = 0 }}
```

And the following expression constraint returns both the active and inactive members of the  `816080008 | International Patient Summary|` .

```
^ 816080008 |International Patient Summary| {{ M active = "*" }}
```

Please note that this filter uses the active status of the reference set member. It does not consider the active status of the component that the member references. For example an active reference set member may refer to an inactive concept.
