...
Description active filters enable an expression constraint to match on only those concepts with a description that has a matching active status. Descriptions are either active (i.e. active = 1 or active = "true"), inactive (i.e. active = 0 or active = "false") or either (i.e. active = "\*"). By default, only active descriptions are included in the substrate.
...
The following expression constraints return all concepts in the International Patient Summary reference set, which have an inactive description.
...
And the following expression constraint returns all concepts in the International Patient Summary reference set, which have either an active or an inactive description.

```
^ 816080008 |International Patient Summary| {{ D active = "*" }}
```
